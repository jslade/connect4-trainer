...
GCWeb.Game.serviceUrl = "http://cloud.cs.berkeley.edu:8090/gcweb/service/gamesman/puzzles/";
...
GCWeb.Game.prototype.start = function() {
  ...
  this.showingMoveValues = true;//$("#option-move-values:checked").length > 0;
  ...
  if (!this.local) {
    ...
    options.success = function(data, textStatus, xhr) {
      if (data && data.status == 'ok') {
        ...
        this.chooseMoveValueDisplay(moveValue);        
...
GCWeb.Prediction.prototype.updatePrediction = function() {
  ...
  if (!moveValue) {
    ...
    if (this.game.player != this.lastPlayer) {
      ...
      // the very first move value, before any move has been played, has
      // no "move" attribute and thus the "value" should not be flipped,
      // as this leads to an incorrect "lose" prediction for blue if we
      // undo moves all the way back to the beginning of the game
      if (moveValue.move) {

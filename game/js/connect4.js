...
ConnectFour.prototype.undoMove = function() {  
  ...
  if (this.moveHistory.length < 2) {
    ...
    this.switchTeams();
    // dispose of the piece about to be played
    if (this.pieces.length > 0) {
      this.currentPiece = this.pieces.pop();
      this.removeMoveListener();
      this.removePieceTrackers();
      this.currentPiece.hide();
    }

    // take back the piece most recently played, ready to be played again
    if (this.pieces.length > 0) {
        this.currentPiece = this.pieces[this.pieces.length - 1];
    } else {
        this.currentPiece = this.createPiece();
    }
    this.currentPiece.moveToRow(-1, 500);
    this.attachMoveListener();
    this.attachPieceTracker(this.currentPiece);  
    
    // adjust the move values & prediction accordingly, without
    // having to repeat the API call over the wire
